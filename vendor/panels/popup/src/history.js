		...
		chrome.history.search(
			...
			function(tree) {
				...
				for (let i = 0, length = tree.length; i < length; i++) {
					...
					let title =
						(item.title && T.escapeHtml(item.title)) || new URL(item.url).host;
					...
					<a style="background-image:url(chrome://favicon/${item.url})" href=${
							item.url
						} title="${item.url}">${title}</a>
					<div class="info">
					...
					try {
						divItem.children[0].addEventListener("click", function(el) {
							...
							chrome.history.deleteUrl(
								{ url: el.target.nextElementSibling.getAttribute("href") },
								function(el) {
									console.log(el);
									console.log(this);
									location.reload();
