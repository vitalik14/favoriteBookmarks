 ...
 */
var search = T.id('search_h');
var removeTextSearch = T.id('remove-text-search_h');
...
removeTextSearch.addEventListener('click', removeTextHistory);
...
function removeTextHistory(el, sort, interval) {
	search.value = '';
	searchHistory('', {sort: search.value, interval: 0});
...
function searchHistory(el, data) {
	...
	timeout = setTimeout(() => {
		chrome.history.search({text: '', maxResults: 1000}, function(tree) {
...
search.value = localStorage['lastSearchHystory'];
